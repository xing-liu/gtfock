...
static void initial_guess (PFock_t pfock, BasisSet_t basis,
                           int ispurif, int rowstart, int rowend,
                           int colstart, int colend, double *D_block, int ldx)
{
    int myrank;
    ...
    if (myrank == 0)
    {
        for (int i = 0; i < CInt_getNumAtoms (basis); i++)
        {
            double *guess;
            int spos;
            int epos;
    ...
    if (1 == ispurif)
    {
        PFock_getMat (pfock, PFOCK_MAT_TYPE_D,
                      rowstart, rowend, colstart, colend, D_block, ldx);
    }
}
                        ...
                        double *D_block, double *F_block)
{
    ...
    if (1 == ispurif)
    {
        PFock_getMat (pfock, PFOCK_MAT_TYPE_F,
                      rowstart, rowend, colstart, colend, F_block, stride);
    }
}
...
int main (int argc, char **argv)
{
    ...
    int purif_offload = 0;
    const char *offload_str = getenv ("GTFOCK_OFFLOAD");
    const char *offload_str2 = getenv ("PURF_OFFLOAD");
    if (offload_str != NULL)
    {
    ...
    if (myrank == 0)
    {
        ...
        if (offload_fraction != 0.0)
        {
            const int mic_numdevs = _Offload_number_of_devices ();
            printf ("  #mic_devices = %d\n", mic_numdevs);
            if (mic_numdevs != 0)
            {
                const int nthreads_mic =
    ...
    for (int iter = 0; iter < niters; iter++)
    {
        ...
        {
            printf
                ("    purification takes %.3lf secs, %d iterations, %.3lf Gflops\n",
                 t2 - t1, it,
