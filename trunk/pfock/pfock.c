...
{
    // allocate pfock
    PFock_t pfock = (PFock_t)ALIGN_MALLOC(sizeof(struct PFock), 64);    
    if (NULL == pfock) {
    ...
    pfock->f_startind =
        (int *)ALIGN_MALLOC(sizeof(int) * (pfock->nshells + 1), 64);
    ...
    pfock->s_startind =
        (int *)ALIGN_MALLOC(sizeof(int) * (pfock->natoms + 1), 64);
                              ...
                              int index, PFock_t pfock)
{
    ...
    NGA_Put(pfock->ga_D, lo, hi, (void *)dmat, ld);
    return PFOCK_STATUS_SUCCESS;
}
...
PFockStatus_t PFock_createCoreHMat(PFock_t pfock, BasisSet_t basis)
{
    ...
    int myrank;
    MPI_Comm_rank (MPI_COMM_WORLD, &myrank);    
    pfock->ga_H = GA_Duplicate(pfock->ga_D, "core hamilton mat");
    if (0 == pfock->ga_H) {
        PFOCK_PRINTF (1, "GA allocation failed\n");
        return PFOCK_STATUS_ALLOC_FAILED;
    }
...
{    
    ...
    int myrank;
    MPI_Comm_rank(MPI_COMM_WORLD, &myrank);    
    pfock->ga_S = GA_Duplicate(pfock->ga_D, "overlap mat");
    if (0 == pfock->ga_S) {
        PFOCK_PRINTF (1, "GA allocation failed\n");
        return PFOCK_STATUS_ALLOC_FAILED;
    }
    ...
    int ga_tmp = GA_Duplicate(pfock->ga_D, "tmp mat");
    if (0 == ga_tmp) {
        PFOCK_PRINTF (1, "GA allocation failed\n");
        return PFOCK_STATUS_ALLOC_FAILED;
    }
    int ga_tmp2 = GA_Duplicate(pfock->ga_D, "tmp mat");
    if (0 == ga_tmp2) {
        PFOCK_PRINTF (1, "GA allocation failed\n");
        return PFOCK_STATUS_ALLOC_FAILED;
    }
    ...
    pfock->ga_X = GA_Duplicate(pfock->ga_D, "X mat");
    if (0 == pfock->ga_X) {
        PFOCK_PRINTF (1, "GA allocation failed\n");
        return PFOCK_STATUS_ALLOC_FAILED;
    }
