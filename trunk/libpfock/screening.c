...
int schwartz_screening (PFock_t pfock, BasisSet_t basis)
{
    ...
    double *sq_values = (double *)ALIGN_MALLOC (sizeof(double) *
        pfock->nshells_row * pfock->nshells_col, 64);
    if (NULL == sq_values)
    {
        return -1;
    }
            ...
            for (int N = startN; N <= endN; N++)
            {
                ...
                if (nints != 0)
                {
                    for (int iM = 0; iM < dimM; iM++)
                    {
                        ...
                        {
                            int index = iM * (dimN*dimM*dimN+dimN) + iN * (dimM*dimN+1);
    ...
    CInt_destroyERD (erd);
    ALIGN_FREE (sq_values);

    // init shellptr
    sq_values = (double *)ALIGN_MALLOC (sizeof(double) * nshells, 64);
    if (NULL == sq_values)
    {
        return -1;
    }
    ...
    pfock->shellptr = (int *)ALIGN_MALLOC (sizeof(int) * (nshells + 1), 64);
    ...
    double maxvalue;  
    pfock->shellvalue  = (double *)ALIGN_MALLOC (sizeof(double) * nnz, 64);
    pfock->shellid  = (int *)ALIGN_MALLOC (sizeof(int) * nnz, 64);
    pfock->shellrid  = (int *)ALIGN_MALLOC (sizeof(int) * nnz, 64);
...
{
    ALIGN_FREE (pfock->shellid);
    ALIGN_FREE (pfock->shellrid);
    ALIGN_FREE (pfock->shellptr);
    ALIGN_FREE (pfock->shellvalue);
}
